...
class SimpleTable extends React.Component {
  constructor(props, context) {
    ...
    let data = [];
    this.props.data.body.forEach((rec, index) => {
      let tempdata = [];
      rec.forEach(entry => {
  ...
  render() {
    ...
    return (
      <div className={classes.root} ref="scrolldiv" onScroll={this.scrollEvent}>
          ...
          <TableBody>
            {this.state.orderedData
              ...
              .map((mapping, index) => {
                ...
                return (
                  <TableRow hover key={startRecord + index}>
                    {cells}
                  </TableRow>
                );
              })}
