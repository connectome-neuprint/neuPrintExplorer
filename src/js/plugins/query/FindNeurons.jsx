...
import { generateCypher } from './CellObjects';
...
${whereConditions}
${directedRoiClause}
// convert the neuron.bodyId to a string so that it can be transfered via JSON without
// losing precision.
RETURN apoc.map.setKey(properties(neuron), 'bodyId', toString(neuron.bodyId)) as neuron, rois
      ...
      .map((row) => {
        ...
        colHeaders.forEach((header) => {
          ...
          if (header.id === 'mitoTotal') {
            const cypher = generateCypher(entry.bodyId, 'WHERE m.type="mitochondrion"');
            const mitoQuery = {
              dataSet: query.ds,
              pluginCode: 'cos',
